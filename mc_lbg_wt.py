...
if __name__=="__main__":
    ...
    with open("mc_lbg_wt.txt","w") as fout:
        ...
        for i in range(bins.size):
            outstr = "#"
            for j in range(bins.size): outstr += " {:8.4f}".format(wcor[i,j])
            fout.write(outstr + "\n")
        fout.write("# {:>8s} {:>15s} {:>15s}\n".\
                   format("th[deg]","wt","dwt"))
        for i in range(bins.size):
            outstr = "{:10.3f} {:15.5e} {:15.5e}".format(bins[i],wavg[i],werr[i])
            fout.write(outstr+"\n")
    #
